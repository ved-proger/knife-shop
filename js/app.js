 ...
 */
    ...
    (() => {
        ...
        function menuInit() {
            if (document.querySelector(".icon-menu")) document.addEventListener("click", (function(e) {
                if (bodyLockStatus && e.target.closest(".icon-menu")) {
                    ...
                    if (document.querySelector(".catalog-open")) document.documentElement.classList.remove("catalog-open");
                    if (document.querySelector("._sub-catalog-open")) document.querySelector("._sub-catalog-open").classList.remove("_sub-catalog-open");
                }
            }));
        }
        function menuClose() {
        ...
        function rangeInit() {
            ...
            if (priceSlider) {
                let rengItem = priceSlider;
                let priceStart = rengItem.querySelector("[data-from]");
                let priceEnd = rengItem.querySelector("[data-to]");
                let rangeLine = rengItem.querySelector("[data-range]");
                let rangeMin = rengItem.querySelector("[data-range-min]");
                rengItem.querySelector("[data-range-max]");
                nouislider.create(rangeLine, {
                    start: [ Number(priceStart.dataset.from), Number(priceEnd.dataset.to) ],
                    connect: true,
                    tooltips: [ wNumb({
                        decimals: 0
                    }), wNumb({
                    ...
                    range: {
                        min: [ Number(rangeMin.dataset.rangeMin) ],
                        max: [ Number(rangeMin.dataset.rangeMax) ]
                    }
                });
            ...
            const totalLength = document.querySelector(".range-total-length");
            if (totalLength) {
                let rengItem = totalLength;
                let priceStart = rengItem.querySelector("[data-from]");
                let priceEnd = rengItem.querySelector("[data-to]");
                let rangeLine = rengItem.querySelector("[data-range]");
                let rangeMin = rengItem.querySelector("[data-range-min]");
                ...
                function setPriceValues() {
                    ...
                    if ("" != priceStart.value) priceStartValue = priceStart.value;
                    if ("" != priceEnd.value) priceEndValue = priceEnd.value;
                    rangeLine.noUiSlider.set([ priceStartValue, priceEndValue ]);
                }
            }
            const bladeLength = document.querySelector(".range-blade-length");
            if (bladeLength) {
                let rengItem = bladeLength;
                let priceStart = rengItem.querySelector("[data-from]");
                let priceEnd = rengItem.querySelector("[data-to]");
                let rangeLine = rengItem.querySelector("[data-range]");
                let rangeMin = rengItem.querySelector("[data-range-min]");
        ...
        flsModules.tippy = tippy_esm("[data-tippy-content]:not(.product-balls__help)", {});
        tippy_esm(".product-balls__help", {
            placement: "top-start",
            arrow: true,
        ...
        function initSliders() {
            if (document.querySelector(".main-block__slider")) new core(".main-block__slider", {
                ...
                pagination: {
                    el: ".controlls-main-slider__bullets",
                    renderBullet: function(index, className) {
                        return `<button type="button" class="${className} slider-bullet\t\t"></button>`;
                    },
                    clickable: true
                },
                on: {
                    init: function(swiper) {
                        const currentEl = document.querySelector(".fraction-mainslider__current");
                        const totalEl = document.querySelector(".fraction-mainslider__total");
                        const allsliderItems = document.querySelectorAll(".slide-main-block:not(.swiper-slide-duplicate)");
                        if (currentEl) currentEl.innerHTML = swiper.realIndex < 10 ? `0${swiper.realIndex + 1}` : swiper.realIndex + 1;
                        if (totalEl) totalEl.innerHTML = "/" + allsliderItems.length;
                    },
            ...
            if (document.querySelector(".slider-product")) new core(".slider-product", {
            ...
            if (document.querySelector(".slider-novelties")) new core(".slider-novelties", {
                modules: [ Autoplay, Pagination ],
                observer: true,
                observeParents: true,
                spaceBetween: 40,
            ...
            if (document.querySelector(".mainSlider-product") && document.querySelector(".subSlider-product")) {
                if (document.querySelector(".mainSlider-product__slide")) {
                    let slides = document.querySelectorAll(".mainSlider-product__slide");
                    for (let i = 0; i < slides.length; i++) {
                        let thumbSlides = document.querySelectorAll(".subSlider-product__slide");
                        thumbSlides[i].setAttribute("data-thumbnail-index", `${i}`);
                    }
                }
                if (document.querySelector(".subSlider-product__slide")) {
                    let thumbnails = document.querySelectorAll(".subSlider-product__slide");
                    thumbnails.forEach((function(item) {
                        item.addEventListener("keydown", (function(e) {
                            if (32 === e.keyCode) {
                                const slideIndex = this.dataset.thumbnailIndex;
                                subSliderProduct.slideTo(slideIndex);
                                mainSliderProduct.slideTo(slideIndex);
                            }
                        }));
                    }));
                }
                let subSliderProduct = new core(".subSlider-product", {
                    modules: [ Thumb ],
                    spaceBetween: 15,
                    slidesPerView: 4,
                    freeMode: true,
                ...
                let mainSliderProduct = new core(".mainSlider-product", {
                    modules: [ Thumb ],
                    keyboard: {
                        enabled: true
                    },
        ...
        da.init();
        let menuBlocks = document.querySelectorAll(".sub-menu-catalog__block");
        if (menuBlocks.length) menuBlocks.forEach((item => {
            let menuCategories = item.querySelectorAll(".sub-menu-catalog__category").length;
            if (menuCategories) item.classList.add(`sub-menu-catalog__block--${menuCategories}`);
        }));
        ...
        function documentActions(e) {
            ...
            if (targetElement.closest("[data-parent")) {
                e.preventDefault;
                const likSubmenu = targetElement.dataset.parent ? targetElement.dataset.parent : null;
                const subMenu = document.querySelector(`[data-submenu='${likSubmenu}']`);
                if (subMenu) {
                    const subMenus = document.querySelectorAll(`[data-submenu]`);
                    subMenus.forEach((elem => {
                        if (subMenu.dataset.submenu != elem.dataset.submenu) elem.classList.remove("_sub-menu-open"); else subMenu.classList.toggle("_sub-menu-open");
                    }));
                } else console.log("нет такго подменю");
            } else if (targetElement.closest(".menu-catalog__link")) if (document.querySelector(".sub-menu-catalog__block._sub-menu-open")) document.querySelector(".sub-menu-catalog__block._sub-menu-open").classList.remove("_sub-menu-open");
            if (targetElement.closest(".menu-catalog__link")) {
                if (document.querySelector(".menu-catalog__item._sub-menu-active")) document.querySelector(".menu-catalog__item._sub-menu-active").classList.remove("_sub-menu-active");
                targetElement.parentElement.classList.add("_sub-menu-active");
            }
            if (targetElement.closest(".menu-top-header__link--catalog")) {
                e.preventDefault();
                document.documentElement.classList.add("catalog-open");
            }
            if (targetElement.closest(".back-catalog")) {
                e.preventDefault();
                if (document.querySelector(".catalog-open")) document.documentElement.classList.remove("catalog-open");
            }
            if (targetElement.closest(".back-sub-catalog")) {
                e.preventDefault();
                if (document.querySelector(".sub-catalog-open")) document.querySelector(".sub-catalog-open").classList.remove("sub-catalog-open");
